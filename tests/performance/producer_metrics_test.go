...
func heldProducer(tb testing.TB, n int) (*gin.Engine, []*library.FluentMsg, *heldSender, chan *library.FluentMsg) {
	...
	for i := range messages {
		...
		messages[i] = m
		in <- m
		if <-first.in != m || <-second.in != m { tb.Fatal("fanout mismatch") }
		first.result <- m
	}
	engine := gin.New()
	...
	for pendingGauge(tb, engine) != int64(n) {
		if time.Now().After(deadline) { tb.Fatal("pending gauge did not settle") }
...
func pendingGauge(tb testing.TB, e *gin.Engine) int64 {
	...
	if err := json.Unmarshal(w.Body.Bytes(), &snapshot); err != nil { tb.Fatal(err) }
	var producer struct { Pending int64 `json:"waitToDiscardMsgNum"` }
	if err := json.Unmarshal(snapshot["producer"], &producer); err != nil { tb.Fatal(err) }
...
func TestPerformanceContractPendingGaugeTracksFanout(t *testing.T) {
	for _, failed := range []bool{false, true} {
		t.Run(fmt.Sprint(failed), func(t *testing.T) {
			...
			if len(commit) != 0 { t.Fatal("incomplete fanout was committed") }
			for _, m := range msgs {
				if failed { second.failed <- m } else { second.result <- m }
			}
			deadline := time.Now().Add(time.Second)
			for pendingGauge(t, e) != 0 {
				if time.Now().After(deadline) { t.Fatal("terminal fanout still counted as pending") }
				time.Sleep(time.Millisecond)
			}
			if failed {
				if len(commit) != 0 { t.Fatal("failed fanout was committed") }
			} else {
				for range msgs {
					select { case <-commit: case <-time.After(time.Second): t.Fatal("fanout did not finish") }
				}
			}
		})
	}
