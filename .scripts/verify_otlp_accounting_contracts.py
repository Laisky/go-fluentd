#!/usr/bin/env python3
"""Reject unsafe destination-accounting variants in disposable source copies.
...
def main():
    ...
    store = "internal/otlpstate/store.go"
    producer = "internal/controller/otlp_producer.go"
    state_test = "TestAcceptedReceiptSurvivesReopen/metrics/application/json"
    mixed = "TestOTLPProducerMixedOutcomesAndJournalRestart/false"
    positive = "TestOTLPProducerFullAcceptanceAndReleaseRetry"
    variants = [
        ("forget-acceptance", store, "if retainAccepted && o.Kind == Accepted {",
         "if false && retainAccepted && o.Kind == Accepted {", state_test,
         "TestTransportFailuresRemainRetryable"),
        ("resend-accepted", store, "if old != nil {",
         "if old != nil && old.Outcome.Kind != Accepted {", state_test,
         "TestTransportFailuresRemainRetryable"),
        ("quarantine-counted-delivered", producer, "p.quarantined.Add(1)",
         "p.accepted.Add(1)", mixed, positive),
        ("release-pending-destination", producer, "if !allResolved {",
         "if false && !allResolved {", mixed, positive),
        ("quarantine-is-full-delivery", producer,
         "case result.Durable && result.Quarantined:\n\t\t\tallAccepted = false",
         "case result.Durable && result.Quarantined:\n\t\t\tallAccepted = true", mixed, positive),
    ]
    original = {f: (source / f).read_text() for f in (store, producer)}
    summary = {"source_sha256": {f: hashlib.sha256(c.encode()).hexdigest()
                                  for f, c in original.items()}}
    with tempfile.TemporaryDirectory(prefix="otlp-accounting-controls-") as tmp:
        root = Path(tmp) / "repo"
        ...
        for name, file, old, new, target, control in variants:
            ...
            summary[name + "-control"] = run(root, package, "^" + control + "$", control,
