...
func NewHTTPRecv(cfg *HTTPRecvCfg) *HTTPRecv {
	libs.Logger.Info("create HTTPRecv",
	...
	if cfg.Path == "" {
		libs.Logger.Panic("path should not be emqty")
	}
...
func (r *HTTPRecv) validate(ctx *gin.Context, msg *libs.FluentMsg) bool {
	...
	case []byte:
	default:
		libs.Logger.Warn("unknown type of timekey", zap.String(r.TimeKey, fmt.Sprint(msg.Message[r.TimeKey])))
		r.BadRequest(ctx, "unknown type of timekey")
		return false
	}

	if !r.TSRegexp.Match(msg.Message[r.TimeKey].([]byte)) {
		libs.Logger.Warn("unknown format of timekey", zap.ByteString(r.TimeKey, msg.Message[r.TimeKey].([]byte)))
	...
	case nil:
		libs.Logger.Warn("`sig` not exists")
	...
	default:
		libs.Logger.Warn("`unknown type of `sig`", zap.String(r.SigKey, fmt.Sprint(msg.Message[r.SigKey])))
	...
	if sig != string(msg.Message[r.SigKey].([]byte)) {
		libs.Logger.Warn("signature of timekey incorrect",
...
func (r *HTTPRecv) BadRequest(ctx *gin.Context, msg string) {
	if err := ctx.AbortWithError(http.StatusBadRequest, fmt.Errorf(msg)); err != nil {
		libs.Logger.Error("abort http", zap.Error(err), zap.String("msg", msg))
	}
}
...
func (r *HTTPRecv) HTTPLogHandler(ctx *gin.Context) {
	...
	case "prod":
	default:
		libs.Logger.Warn("unknown env", zap.String("env", env))
		r.BadRequest(ctx, fmt.Sprintf("only accept sit/perf/uat/prod, but got `%v`", env))
		return
	}
	// libs.Logger.Debug("got new http log", zap.String("env", env))

	if ctx.Request.ContentLength > r.MaxBodySize {
		libs.Logger.Warn("content size too big", zap.Int64("size", ctx.Request.ContentLength))
